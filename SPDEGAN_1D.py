...
from fourier_space1d_time import FNO_layer 
...
class MLP1D(torch.nn.Module):
    def __init__(self, in_size, out_size, mlp_size, num_layers, tanh):
        super().__init__()

        model = [torch.nn.Conv1d(in_size, mlp_size, 1),
                 LipSwish()]
        for _ in range(num_layers - 1):
            model.append(torch.nn.Conv1d(mlp_size, mlp_size, 1))
            ###################
            # LipSwish activations are useful to constrain the Lipschitz constant of the discriminator.
            # (For simplicity we additionally use them in the generator, but that's less important.)
            ###################
            model.append(LipSwish())
        model.append(torch.nn.Conv1d(mlp_size, out_size, 1))
        if tanh:
            model.append(torch.nn.Tanh())
        self._model = torch.nn.Sequential(*model)
...
class Generator(torch.nn.Module):  
    def __init__(self, ic, wiener, data_size, initial_noise_size, noise_size, hidden_size, mlp_size, num_layers, modes1, modes2, T):
        super().__init__()
        ...
        self._initial = MLP1D(initial_noise_size, hidden_size, mlp_size, num_layers, tanh=False)
        self._func = GeneratorFunc(noise_size, hidden_size, mlp_size, num_layers)
        self._readout = torch.nn.Conv2d(hidden_size, data_size, 1)
...
class DiscriminatorSPDE(torch.nn.Module): 
    def __init__(self, data_size, hidden_size, mlp_size, num_layers, modes1, modes2, T):
        super().__init__()

        self._readin = MLP1D(data_size, hidden_size, mlp_size, num_layers, tanh=False)
        self._func = GeneratorFunc(data_size, hidden_size, mlp_size, num_layers)
        self._net =  NeuralFixedPoint(self._func, modes1, modes2, T, n_iter=2)
    ...
    def forward(self, x):
        ...
        x = self._readout(x)  # (batch, 1, dim_x, dim_t)
...
class Discriminator(torch.nn.Module): 
    def __init__(self, data_size, hidden_size, mlp_size, num_layers, modes1, modes2):
        super().__init__()

        self._readin = MLP(data_size+2, hidden_size, mlp_size, num_layers, tanh=False)
        self._net =  [ FNO_layer(modes1, modes2, hidden_size) for i in range(num_layers-1) ]
        self._net += [ FNO_layer(modes1, modes2, hidden_size, last=True) ]
    ...
    def get_grid(self, shape, device):
        ...
        return torch.cat((gridx, gridt), dim=-1).permute(0,3,1,2).to(device)
