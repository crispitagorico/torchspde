...
def dataloader_ncdeinf_1d(u, xi, ntrain=1000, ntest=200, T=51, sub_t=1, batch_size=20, dim_x=128, normalizer=True, interpolation='linear', dataset=None):
    ...
    u_train = u[:ntrain, :dim_x, :T:sub_t].permute(0, 2, 1).unsqueeze(1) #(N, 1, dim_t, dim_x)
    xi_train = xi[:ntrain, :dim_x, :T:sub_t].permute(0, 2, 1).unsqueeze(1) #(N, 1, dim_t, dim_x)
    ...
    u_test = u[-ntest:, :-1, :T:sub_t].permute(0, 2, 1).unsqueeze(1) 
    xi_test = xi[-ntest:, :-1, :T:sub_t].permute(0, 2, 1).unsqueeze(1) 
...
def train_ncdeinf_1d(model, train_loader, test_loader, device, loss, batch_size=20, epochs=5000, learning_rate=0.001, scheduler_step=100, scheduler_gamma=0.5, print_every=20):
